...
from models.model_dnabert import FullModel
...
# Define output name
outputname = "dnabert_experiment"  # Change this to your desired output name

# Load data
batch_size = 32
tokenizer = AutoTokenizer.from_pretrained("zhihan1996/DNABERT-2-117M", trust_remote_code=True)
train_loader, tokens1_test, tokens2_test, eff_test = load_data("pentamer5MLx.csv", tokenizer, LLM_type='DNA', undersample_high_eff=0.3, test_size=0.15, batch_size=batch_size, device=device)
...
for fold, (train_idx, val_idx) in enumerate(kf.split(train_loader.dataset)):
    ...
    # Unfreeze DNABert parameters if they were frozen
    for param in dna_bert.parameters():
        param.requires_grad = True

    model = FullModel(dna_bert, input_size).to(device)
